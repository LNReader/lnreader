      ...
      <Checkbox
        ...
        onPress={() =>
          filter === 'chaptersUnread IS NULL'
            ? dispatch(filterLibrary(sort, ''))
            : dispatch(filterLibrary(sort, 'chaptersUnread IS NULL'))
        }
      />
    </View>
  );
    ...
    <View style={{flex: 1}}>
      {sortOrders.map((item, index) => (
        <SortItem
          ...
          theme={theme}
          status={
            sort === item.ASC ? 'asc' : sort === item.DESC ? 'desc' : null
          }
          onPress={() =>
            sort === item.ASC
  ...
  const ThirdRoute = () => (
    <View style={{flex: 1}}>
      ...
      <Checkbox
        ...
        onPress={() =>
          dispatch(setAppSettings('showDownloadBadges', !showDownloadBadges))
        }
        theme={theme}
      />
      <Checkbox
        ...
        onPress={() =>
          dispatch(setAppSettings('showUnreadBadges', !showUnreadBadges))
        }
        theme={theme}
      />
      <Checkbox
        ...
        onPress={() =>
          dispatch(setAppSettings('showNumberOfNovels', !showNumberOfNovels))
        }
        theme={theme}
      />
    </View>
